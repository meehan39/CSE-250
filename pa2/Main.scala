...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val streets = new ArrayBuffer[String]


    val filename = "data/2017-2018_Assessment_Roll-updated-small.csv"

    var numLines = 25
    println("Original streets:")
    println("-----")
    for (entry <- loadEntries(filename, numLines)) {
      taxEntryStore.insert(entry)
      println(entry.infoMap("STREET"))
    }
    println("-----")

    println(taxEntryStore.length)
//    for (grouping <- groupings){
//      println(grouping.value.toString)
//    }
//    println(taxEntryStore.toString)
    val iter = taxEntryStore.iterator
    while (iter.hasNext){
      val entry = iter.next()
      streets.addOne(entry.infoMap("STREET"))
    }
